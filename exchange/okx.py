"""
OKX Futures exchange implementation using ccxt async.
"""
...
class OKXExchange(BaseExchange):
    """Adapter for OKX Futures (perpetual swaps)."""
    ...
    async def fetch_position(self, symbol: str) -> Position:
        ...
        if not positions:
            return Position(
                ...
                margin=0.0, timestamp=datetime.utcnow(), exchange="okx"
            )
        p = positions[0]
        return Position(
            symbol=p["symbol"],
    ...
    async def fetch_balance(self) -> Balance:
        ...
        return Balance(
            total=total, free=free, used=used, currency="USDT", exchange="okx"
        )
